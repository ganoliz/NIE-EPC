...
* For implementation use language dialect (standard) *{cpp}11, {cpp}14, {cpp}17, or {cpp}20*.
...
- *Link to this repository* is https://gitlab.fit.cvut.cz/NI-EPC/B211/USERNAME, where *USERNAME* is you faculty username.
...
WARNING: The *final submission* is considered the *version of the `optional.h` header file present in the repository at this deadline date*. *Any further updates might not be taken into consideration for the classification.*
...
* In case that *it will not be possible to compile* the submitted solution (its compilation will result in an error), the classification will be *base on the _“code review”_ (that is, based on its visual examination).
** In such a case, there is *no possibility to define objective criteria for points assessment*, and this will be made *on a subjective basis without any point guarantees*.
